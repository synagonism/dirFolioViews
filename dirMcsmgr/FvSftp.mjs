/**
 * DOING: sftp the-files in dirMcsmgr/FvSftpA.json
 * INPUT:
 * OUTPUT:
...
function fSftp () {
  var aFil = JSON.parse(moFs.readFileSync('dirMcsmgr/FvSftpA.json'))
  console.log(aFil)
  ...
  const fSend_file = (oConfigIn, sFileIn) => {
      ...
      sftp.connect(oConfigIn).then(() => {
        // SPECIFIC INFO
        return sftp.put("C:/xampp/htdocs/dWstSgm/dirFolioViews/" + sFileIn,
          "/var/www/vhosts/synagonism.net/httpdocs/dirFolioViews/" + sFileIn);
      }).then(() => {
